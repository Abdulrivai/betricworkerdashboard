...
import WorkerModals from './components/WorkerModals';
...
export default function WorkersPage() {
  ...
  const handleCreateWorker = async (email: string, fullName: string, password: string) => {
    try {
      ...
      if (response.ok) {
        ...
        await Swal.fire({
          title: '❌ Gagal',
          text: data.error || 'Gagal membuat worker',
          icon: 'error',
          confirmButtonColor: '#dc2626',
          confirmButtonText: 'OK',
          customClass: {
      ...
      await Swal.fire({
        title: '❌ Error',
        text: 'Terjadi kesalahan saat membuat worker',
        icon: 'error',
        confirmButtonColor: '#dc2626',
        confirmButtonText: 'OK',
        customClass: {
  ...
  const handleUpdateWorker = async (workerId: string, email?: string, password?: string) => {
    try {
      const body: any = {};
      ...
      if (response.ok) {
        ...
        await Swal.fire({
          title: '❌ Gagal',
          text: data.error || 'Gagal mengupdate worker',
          icon: 'error',
          confirmButtonColor: '#dc2626',
          confirmButtonText: 'OK',
          customClass: {
      ...
      await Swal.fire({
        title: '❌ Error',
        text: 'Terjadi kesalahan saat mengupdate worker',
        icon: 'error',
        confirmButtonColor: '#dc2626',
        confirmButtonText: 'OK',
        customClass: {
  ...
  const handleDeleteWorker = async (worker: Worker) => {
    const result = await Swal.fire({
      ...
      html: `Apakah Anda yakin ingin menghapus worker:<br><strong>${worker.full_name}</strong> (${worker.email})?<br><br><span class="text-red-600">Data akan dihapus permanen!</span>`,
      icon: 'warning',
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Handle active projects error specially
        if (response.status === 409 && data.activeProjects) {
          await Swal.fire({
            title: '❌ Tidak Dapat Dihapus',
            html: `${data.error}<br><br>Project aktif:<br>${data.activeProjects.map((p: any) => `- ${p.title} (${p.status})`).join('<br>')}`,
            icon: 'error',
            confirmButtonColor: '#dc2626',
            confirmButtonText: 'OK',
            customClass: {
          ...
          await Swal.fire({
            title: '❌ Gagal',
            text: data.error || 'Gagal menghapus worker',
            icon: 'error',
            confirmButtonColor: '#dc2626',
            confirmButtonText: 'OK',
            customClass: {
      ...
      await Swal.fire({
        title: '❌ Error',
        text: 'Terjadi kesalahan saat menghapus worker',
        icon: 'error',
        confirmButtonColor: '#dc2626',
        confirmButtonText: 'OK',
        customClass: {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50">
      {/* Header */}
      <header className="bg-white/80 backdrop-blur-md border-b border-blue-900/10">
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-4">
              <button
                ...
                className="bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white px-6 py-3 rounded-xl font-bold transition-all duration-300 shadow-lg hover:shadow-2xl hover:scale-105 flex items-center space-x-2"
              >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col space-y-2">
                          <button
                            ...
                            className="bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 text-white px-4 py-2 text-sm rounded-lg transition-all duration-200 font-semibold shadow-md hover:shadow-lg hover:scale-105 flex items-center justify-center space-x-1"
                          >
                          ...
                          <button
                            ...
                            className="bg-gradient-to-r from-red-500 to-rose-600 hover:from-red-600 hover:to-rose-700 text-white px-4 py-2 text-sm rounded-lg transition-all duration-200 font-semibold shadow-md hover:shadow-lg hover:scale-105 flex items-center justify-center space-x-1"
                          >
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <button
                            ...
                            className="bg-gradient-to-r from-blue-500 to-cyan-600 hover:from-blue-600 hover:to-cyan-700 text-white px-4 py-2 text-sm rounded-lg transition-all duration-200 font-semibold shadow-md hover:shadow-lg hover:scale-105 flex items-center justify-center space-x-1"
                          >
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <Link href={`/worker?worker_id=${worker.id}`} target="_blank">
                            <button className="w-full bg-gradient-to-r from-purple-500 to-pink-600 hover:from-purple-600 hover:to-pink-700 text-white px-4 py-2 text-sm rounded-lg transition-all duration-200 font-semibold shadow-md hover:shadow-lg hover:scale-105 flex items-center justify-center space-x-1">
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <WorkerModals
        ...
        selectedWorker={selectedWorkerForEdit}
        setSelectedWorker={setSelectedWorkerForEdit}
